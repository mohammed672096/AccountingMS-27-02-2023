...
namespace PosFinalCost
{
    public partial class ReportProExpirte : XtraReport
    {
        ...
        private void StoreName_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;
            cell.Text = Session.Stores.FirstOrDefault(x => x.ID == Convert.ToInt16(cell.Value))?.Name;
        }

        private void xrtcPrdNo_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;
            cell.Text = Session.Products.FirstOrDefault(x => x.ID == Convert.ToInt32(cell.Value))?.No;
        }

        private void xrtcPrdName_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;
            cell.Text = Session.Products.FirstOrDefault(x => x.ID == Convert.ToInt32(cell.Value))?.Name;
        }

        private void xrtcMsurName_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;

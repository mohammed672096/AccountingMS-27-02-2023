...
namespace PosFinalCost
{
    public partial class ComponentFlyoutDialog : Component
    {
        ...
        public void ShowDialogUC(UserControl ucOwner, string mssg, bool isNew = true)
        {
            ...
            simpleLabelItem1.Text = $"{(!Program.My_Setting.LangEng ? (isNew ? "تم حفظ " : "تم تعديل بيانات ") + mssg + " بنجاح!" : mssg + (isNew ? " created successfully!" : " updated successfully!"))}";
            ShowFlyoutPanel();
        }
        ...
        public void ShowDialogUCUpdMsg(UserControl ucOwner, string message)
        {
            ...
            flyoutPanel1.Options.AnimationType = DevExpress.Utils.Win.PopupToolWindowAnimation.Slide;
            flyoutPanel1.OwnerControl = ucOwner;
            simpleLabelItem1.Text = (!Program.My_Setting.LangEng) ? "تم تعديل " + message + " بنجاح " : message + " updated successfully";
            ShowFlyoutPanel();
        }
        ...
        public void WaitForm(UserControl ucOwner, byte status)
        {
            ...
            else if (status == 1) this.overlaySplashScreen = SplashScreenManager.ShowOverlayForm(ucOwner);
        }

...
namespace PosFinalCost.Classe
{
    public class _Currency
    {
        ...
        public string Name { get; set; } = "ريال سعودي";
        public string Pound1 { get; set; } = "ريال سعودي";
        public string Pound1En { get; set; } = "Saudi Reyal";
        public string Pound2 { get; set; } = "ريالان سعوديان";
        public string Pound2En { get; set; } = "Saudi Reyals";
        public string Pound3 { get; set; } = "ريالا سعوديا";

        public string Pound3En { get; set; } = "Saudi Reyals";


        public string Piaster1 { get; set; } = "هلله";

        public string Piaster1En { get; set; } = "Halala";


        public string Piaster2 { get; set; } = "هللتان";

        public string Piaster2En { get; set; } = "Halalas";


        public string Piaster3 { get; set; } = "هلله";

        public string Piaster3En { get; set; } = "Halalas";

        public double LastRate { get; set; }

        public Byte PartPrecision { get; set; } = 2;
        public Boolean Is_CurrencyNameFeminine { get; set; } = false;
        public Boolean Is_CurrencyPartNameFeminine { get; set; } = true;
    ...
    public class ToWord
    {
        ...
        /// <param name="_Currency">_Currency to use</param>
        public ToWord(double number)
        {
            InitializeClass((double)number, String.Empty, "only.", "فقط", "لا غير.");
        }
        ...
        public ToWord(double number, String englishPrefixText, String englishSuffixText, String arabicPrefixText, String arabicSuffixText)
        {
            InitializeClass((double)number, englishPrefixText, englishSuffixText, arabicPrefixText, arabicSuffixText);
        }
        ...
        /// <param name="arabicPrefixText">Arabic text to be placed before the generated text</param>
        /// <param name="arabicSuffixText">Arabic text to be placed after the generated text</param>
        private void InitializeClass(double number, String englishPrefixText, String englishSuffixText, String arabicPrefixText, String arabicSuffixText)
        {
            Number = number;
            _Currency = new _Currency();
        ...
        private string GetDecimalValue(string decimalPart)
        {
            ...
            if (_Currency.PartPrecision != decimalPart.Length)
            {
                int decimalPartLength = decimalPart.Length;

                for (int i = 0; i < _Currency.PartPrecision - decimalPartLength; i++)
                {
                    decimalPart += "0"; //Fix for 1 number after decimal ( 10.5 , 1442.2 , 375.4 ) 
                }

                result = String.Format("{0}.{1}", decimalPart.Substring(0, _Currency.PartPrecision), decimalPart.Substring(_Currency.PartPrecision, decimalPart.Length - _Currency.PartPrecision));
            ...
            for (int i = 0; i < _Currency.PartPrecision - result.Length; i++)
            {
                result += "0";
            }
        ...
        /// <summary>
        /// Convert stored number to words using selected _Currency
        /// </summary>
        /// <returns></returns>
        public string ConvertToEnglish()
        {
            ...
            formattedNumber += (retVal != String.Empty) ? (_intergerValue == 1 ? _Currency.Pound1En : _Currency.Pound3En) : String.Empty;
            ...
            formattedNumber += (decimalString != String.Empty) ? " " + (_decimalValue == 1 ? _Currency.Piaster1En : _Currency.Piaster3En) : String.Empty;
        ...
        private string GetDigitFeminineStatus(int digit, int groupLevel)
        {
            ...
            { // if it is in the decimal part
                if (_Currency.Is_CurrencyPartNameFeminine)
                ...
                if (groupLevel == 0)
            {
                if (_Currency.Is_CurrencyNameFeminine)
                    return arabicFeminineOnes[digit];// use feminine field
                else
                    return arabicOnes[digit];
            }
        ...
        /// <param name="groupNumber">The group number to process</param>
        /// <returns></returns>
        private string ProcessArabicGroup(int groupNumber, int groupLevel, double remainingNumber)
        {
            int tens = groupNumber % 100;
        ...
        public string ConvertToArabic()
        {
            ...
            formattedNumber += (retVal != String.Empty) ? retVal : String.Empty;
            if (_intergerValue != 0)
            { // here we add _Currency name depending on _intergerValue : 1 ,2 , 3--->10 , 11--->99
                int remaining100 = (int)(_intergerValue % 100);

                if (remaining100 == 0)
                    formattedNumber += _Currency.Pound1;
                else
                    if (remaining100 == 1)
                    formattedNumber += _Currency.Pound1;
                else
                ...
                {
                    if (_intergerValue == 2)
                        formattedNumber += _Currency.Pound2;
                    else
                        formattedNumber += _Currency.Pound1;
                }
                else
                            if (remaining100 >= 3 && remaining100 <= 10)
                    formattedNumber += _Currency.Pound3;
                else
            ...
            if (_decimalValue != 0)
            { // here we add _Currency part name depending on _intergerValue : 1 ,2 , 3--->10 , 11--->99
                formattedNumber += " ";

                int remaining100 = (int)(_decimalValue % 100);

                if (remaining100 == 0)
                    formattedNumber += _Currency.Piaster1;
                else
                    if (remaining100 == 1)
                    formattedNumber += _Currency.Piaster1;
                else
                        if (remaining100 == 2)
                    formattedNumber += _Currency.Piaster2;
                else
                            if (remaining100 >= 3 && remaining100 <= 10)
                    formattedNumber += _Currency.Piaster3;
                else
                                if (remaining100 >= 11 && remaining100 <= 99)
                    formattedNumber += _Currency.Piaster3;
            }

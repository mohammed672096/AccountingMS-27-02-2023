...
namespace AccountingMS
{
    public partial class formBarcode : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void InitDefaultData()
        {
            ...
            textEditCompanyName.EditValue = Session.CurBranch.brnName;
        }
        ...
        public  void SearchProduct(string barcodeNo)
        {
            ...
            var barcode = Session.tblBarcode.FirstOrDefault(x => x.brcNo == barcodeNo);
            if (barcode == null) return;
            var PriceMsur = Session.tblPrdPriceMeasurment.FirstOrDefault(x => x.ppmId == barcode.brcPrdMsurId);

            textEditPrdMsur.EditValue = barcode.brcPrdMsurId;
        ...
        private bool ValidateBarcodeNo(string barcodeNo)
        {
            ...
            bool isValid = Session.tblBarcode.Any(x => x.brcNo == barcodeNo);
            if (!isValid)
        ...
        private void BarCodeText_EditValueChanged(object sender, EventArgs e)
        {
            ...
            var pro =Session.Products.FirstOrDefault(x=>x.id==Convert.ToInt32(editor?.EditValue));
            if (pro == null) return;
            textEditProductName.EditValue = MySetting.GetPrivateSetting.LangEng&& pro.prdNameEng!=null ? pro.prdNameEng: pro.prdName;
        ...
        private void TextEditPrdMsur_EditValueChanged(object sender, EventArgs e)
        {
            ...
            {
                var tbPrdMsur = Session.tblPrdPriceMeasurment.FirstOrDefault(x => x.ppmId == ppmId);
                if (tbPrdMsur == null) return;
                var barcode = Session.tblBarcode.FirstOrDefault(x => x.brcPrdMsurId == tbPrdMsur.ppmId);
                textEditBarocdeNo.EditValue = barcode.brcNo;
                bool proTax = (Session.Products.AsQueryable().FirstOrDefault(x => x.id == tbPrdMsur.ppmPrdId)?.prdPriceTax)??false;
                textEditPrdPrice.EditValue =GetppmSalePrice(proTax,tbPrdMsur);
            }
        ...
        {
            tblPrdPriceMeasurmentBindingSource.DataSource = Session.tblPrdPriceMeasurment.Where(x => x.ppmPrdId == prdId).ToList();
            textEditPrdMsur.EditValue = Session.tblPrdPriceMeasurment.FirstOrDefault(x => x.ppmPrdId == prdId && x.ppmDefault == true)?.ppmId;
        }
        ...
        private IDictionary<BarcodeSettings, string> GetDictionarySettings()
        {
            ...
            if (checkEditPrdPrice.Checked && !string.IsNullOrEmpty(textEditPrdPrice.Text))
            {
                ...
                if (MySetting.GetPrivateSetting.LangEng)
                    ...
                    barcodeSettings.Add(BarcodeSettings.Price, (!checkEditSaleTax.Checked ? $"السعر: {price:n2}" : $"السعر شامل الضريبة: {priceTax}"));
            }

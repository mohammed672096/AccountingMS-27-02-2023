...
namespace AccountingMS
{
    public partial class UC_Master : UserControl
    {
        ...
        private void GetResources()
        {
            ...
            foreach (var control in bindingNavigator.Items)
                if (control is ToolStripItem c)
                    _resource.ApplyResources(c, c.Name, _ci);
        }
        private void EnglishTranslation()=>this.RightToLeft = System.Windows.Forms.RightToLeft.No;
        public bool IsNew;
        public string MessString;
        public static string No;
        public static string ErrorText=>"هذا الحقل مطلوب";
        ...
        public static string MessgTextDelete=>!MySetting.GetPrivateSetting.LangEng ? $"هل انت متاكد من حذف  {No} ?" : $"Are you sure to delete {No} ?";
        ...
        private GridControl GetGridControl()
        {
            foreach (var item in this.Controls)
                if (item is DataLayoutControl bbi)
                    return (bbi.Items.Where(x => x is LayoutControlItem layoutControl
                    && layoutControl.Control is GridControl grid).ToList().FirstOrDefault() as LayoutControlItem)?.Control as GridControl;
            return null;
        }
        public virtual void Print()=>GetGridControl()?.ShowPrintPreview();
        ...
        public virtual void ShowError()=>ClsXtraMssgBox.ShowError(MessgTextValidNo);
        ...
        public virtual void Close()=> this.Parent.Dispose();

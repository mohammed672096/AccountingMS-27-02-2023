...
namespace AccountingMS
{
    public partial class FormMain : RibbonForm
    {
        ...
        public FormMain(Boolean _isSale = false)
        {
            ...
            //StaticSession.LoadAll();
            stopWatch.Stop("===========================ElappsedTime staticSession");
            InitializeComponent();
            TrailRemainingDays();
            ...
            var tmp = Session.CurrentYear.fyDateEnd;
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            //if (MySetting.DefaultSetting.AutoUploadDataToMain && !UploadDataToMain.upDataState)
            //{
            //        UploadDataToMain updata = new UploadDataToMain();
            //        TheUpload = new Thread(updata.UpLoadData);
            //        TheUpload.IsBackground = true;
            //        TheUpload.Start();
            //}
        }
        ...
        private void CreateFolderDirectory()
        {
            ...
            Directory.CreateDirectory(@$"{ClsDrive.Path}:\B-Tech\TempSupplyRcpt");
        ...
        protected internal async void SetUserControl(string name)
        {
            try
            {
                switch (name)
                {
                    ...
                    case "barButtonItemSupplier":
                        ...
                        tabpage = new XtraTabPage() { Text = _resource.GetString($"{name}.Caption"), Name = name };
                        break;
                    case "barButtonItem31":
                        //فتح يومية صندوق
                        //frm = new frmDrawerPeriod(true);
                        //frm.ShowDialog();
                        //return;
                    case "barButtonItem32":
                        //اغلاق يومية صندوق
                        //frm = new frmDrawerPeriod(false);
                        //frm.ShowDialog();
                        //return;
                    case "barButtonItem33":
                        //مراجعة يومية صندوق
                        frm = new FormViewDrawer();
                        frm.ShowDialog();
                        return;
                    case "barButtonItem34":
                        ////مراجعة يومية صندوق
                        //frm = new ReportMostActiveSuppliers();
                        //frm.ShowDialog();
                        //return;
                    ...
                    case "barButtonItem36"://تقرير عمولة مندوب (اجمالى المبيعات)
                        frm = new frmReptotalselmandob(); frm.Show(); return;
                    ...
                    case "barButtonItemRprtOrders":
                        frm = frm = new ReportForm(ReportType.Orders);
                        frm.Show(); return;
        ...
        private void EnglishResources()
        {
            ...
            //bsiNavigationPage.Caption = _resource.GetString("barButtonItemAccount.Caption", _ci);
            //bsiNavigationPage.Caption = _resource.GetString("barButtonItemAccount.Caption", _ci);

            //ribbonPageGroup.Text = _resource.GetString("ribbonPageGroup", _ci);
            //bbiCompactMode.Caption = _resource.GetString("bbiCompactMode", _ci);
            //bbiHideMenuCaption.Caption = _resource.GetString("bbiHideMenuCaption", _ci);
            //bbiTouchUI.Caption = _resource.GetString("bbiTouchUI", _ci);
            //bsiTouchScaleFactor.Caption = _resource.GetString("bsiTouchScaleFactor", _ci);
        }

        Font fontConverter => (Font)converter.ConvertFromString(MySetting.GetPrivateSetting.SystemFont);
        ...
        private void barButtonItem1_ItemClick(object sender, ItemClickEventArgs e)
        {
            using (FontDialog dialog = new FontDialog())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    MySetting.GetPrivateSetting.SystemFont = converter.ConvertToString(dialog.Font);
                    MySetting.WriterSettingXml();
                    SetFont();
                }
            }
        ...
        {
            //using var frm = new formSupplyMain(null, SupplyType.Sales);
            //frm.ResetAppearance();RefreshListDialog
            //frm.ShowDialog();
            try
            {
